...
export class AuthRequiredGuard implements CanActivate, OnModuleInit {
  ...
  async canActivate(context: ExecutionContext) {
    this.logger.log(
      chalk.hex(process.env.GUARDS_LOG_COLOR)('--> AuthRequired START'),
    );

    const req = GqlExecutionContext.create(context).getContext<any>()
      .req as Request;
    ...
    if (isAuthHeaderAndIsStatsWithBearer)
    ...
    this.logger.log(
      chalk.hex(process.env.GUARDS_LOG_COLOR)('--> AuthRequired END'),
    );
